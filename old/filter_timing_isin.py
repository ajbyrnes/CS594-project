...
def filter1a(df, sketch):
    return df[df['mjd_annot'].cat.codes.isin(sketch)]

# Filter data with sketch and integer column
def filter1b(df, sketch):
    return df[df['mjd_annot_int'].isin(sketch)]

def filter2a(df, sketch):
    return df[np.isin(df['mjd_annot'].cat.codes.values, sketch)]

def filter2b(df, sketch):
    return df[np.isin(df['mjd_annot_int'].values, sketch)]

def filter3a(df, sketch):
    return df.loc[df['mjd_annot'].cat.codes.isin(sketch)]

def filter3b(df, sketch):
    return df.loc[df['mjd_annot_int'].isin(sketch)]

def filter4a(df, sketch):
    return df.query('mjd_annot.cat.codes in @sketch', engine='numexpr')

def filter4b(df, sketch):
    return df.query('mjd_annot_int in @sketch', engine='numexpr')
